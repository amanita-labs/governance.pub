...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/dreps/${encodeURIComponent(drepId)}/delegators`, {
      next: { revalidate: 60 },
    });
    ...
    return NextResponse.json({ error: 'Failed to fetch DRep delegators' }, { status: 500 });
  }
}
